...
fun CrossFadeExampleAnimation() {
    ...
    Column(Modifier.fillMaxSize()) {
        Button(onClick = { myComponentType = getMyComponentType() }) {
            Text(text = "Cambiar Componente")
        }
    }

    Crossfade(
        targetState = myComponentType, label = "",
    ){
        when (it) {
            ComponentType.Image -> {
                Icon(Icons.Default.SensorDoor, contentDescription = "")
            }
...
fun getMyComponentType(): ComponentType {
